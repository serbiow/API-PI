...
function displaySchedules(schedules) {
    const container = document.getElementById('tbody-content');
    container.innerHTML = ''; // Limpa qualquer conteúdo anterior

    if (schedules.length === 0) {
        container.innerHTML = '<tr><td colspan="6" class="text-center p-2">Nenhum agendamento encontrado</td></tr>';
        return;
    }
    ...
    schedules.forEach(schedule => {
        ...
        t_tr.innerHTML = `
            <td class="p-2">${schedule.serviceName}</td>
            <td class="p-2">${schedule.date}</td>
            <td class="p-2">${schedule.time}</td>
            <td class="p-2 userInfo client-info" style="display: none">${schedule.userName}</td>
            <td class="p-2 userInfo client-info" style="display: none">${schedule.phone}</td>
            <td class="p-2">
